...
class Rotor {
    constructor(rotStr, setting, position) {
        ...
        // Map index into turnover position
        var arrTurn  = [];
     ...
     */
    rotate() {
        // Increment rotor and wraps back around if "overflowed": (N+1) Mod 26
    ...
    */
     ...
     * @param {*} letter The letter traveling into wire
     * @param {*} first If the current is going L <- R ( first time )
     * @return Mapped letter
     */
    lWire(letter, first) {
        var newLetter;
        if (first) { // First travel through rotor (L <- R)
            newLetter = this.sub[letter];
        } else {// Second travel through rotor (L -> R)
            // IndexOf for arrays
     ...
     * @param {*} letter The letter boing outputted
     * @return newLetter
     */
    lOut(letter) {
        const newLetter = (letter + this.setting - this.position + 26) % 26;
